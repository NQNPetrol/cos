                    ...
                    <div class="p-6">
                        <form wire:submit.prevent="{{ $editMode ? 'update' : 'store' }}">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <!-- Título -->
                                 <div class="md:col-span-2">
                                    ...
                                    <input type="text" class="w-full px-3 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500/30 focus:border-blue-500 text-sm" id="titulo" wire:model="titulo">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea class="w-full px-3 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500/30 focus:border-blue-500 text-sm" id="descripcion" rows="4" wire:model="descripcion"></textarea>
                                ...
                                <!-- Categoría -->
                                 <div>
                                    ...
                                    <select class="w-full px-3 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500/30 focus:border-blue-500 text-sm" 
                                            id="categoria" 
                                ...
                                <div>
                                    ...
                                    <select class="w-full px-3 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500/30 focus:border-blue-500 text-sm" id="prioridad" wire:model="prioridad">
                            ...
                            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-zinc-700/50">
                                ...
                                <button type="submit" 
                                        class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl text-white text-sm font-medium transition-all shadow-lg shadow-blue-600/20">
                            ...
                            <table class="w-full table-auto">
                                <thead>
                                    <tr class="border-b border-zinc-700/50">
                                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Título</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Categoría</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Prioridad</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Estado</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Creado por</th>
                                        @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('operador'))
                                            <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Asignado a</th>
                                        @endif
                                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Fecha creación</th>
                                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Fecha cierre</th>
                                ...
                                <tbody class="divide-y divide-zinc-700/30">
                                    @foreach ($tickets as $ticket)
                                        <tr class="hover:bg-zinc-700/30 transition-colors duration-150">
                                            ...
                                            <td class="px-5 py-4 text-white">{{ $ticket->user->name }}</td>
                                            @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('operador'))
                                                <td class="px-5 py-4 text-gray-300">
                                                    {{ $ticket->assignedTo->name ?? 'Sin asignar' }}
                                                </td>
                                            @endif
                                            <td class="px-5 py-4 text-gray-400">{{ $ticket->created_at->format('d/m/Y H:i') }}</td>
                                                ...
                                                <div class="flex space-x-2">
                                                    ...
                                                    @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('operador'))
                                                        <div class="relative group">
                                                            <button class="p-1.5 text-gray-400 hover:text-gray-200 transition-colors" title="Opciones">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"/>
                                                                </svg>
                                                            </button>
                                                            <div class="absolute right-0 mt-1 w-48 bg-zinc-900 border border-zinc-700/50 rounded-xl shadow-2xl opacity-0 invisible transition-all duration-300 group-hover:opacity-100 group-hover:visible z-10">
                                                                <div class="py-1">
