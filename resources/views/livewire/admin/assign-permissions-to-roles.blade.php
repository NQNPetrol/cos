        ...
        <div class="flex items-end gap-3">
            <div class="flex-1">
                ...
                <select wire:model="currentRoleId" wire:change="loadPermissions($event.target.value)"
                        class="bg-zinc-800 border-zinc-700 text-gray-200 rounded px-3 py-2 w-full">
                ...
                @foreach ($permissions as $permission)
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" wire:model="selectedPermissions" value="{{ $permission->name }}"
                               class="rounded border-zinc-600 bg-zinc-800 text-emerald-500 focus:ring focus:ring-emerald-500">
