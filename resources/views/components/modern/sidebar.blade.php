...
<aside class="modern-sidebar" id="modernSidebar" x-data="{ currentLevel: 'main', currentDashboard: 'home' }" role="navigation" aria-label="Menú lateral">
    ...
    <div class="modern-sidebar-logo">
        @if($isClient)
            @php
                $user = auth()->user();
                $logoUrl = $user->logo_cliente ?? asset('cyh.png');
            @endphp
            <img src="{{ $logoUrl }}" alt="Logo" style="max-height: 80px; width: auto;">
        @else
            <img src="{{ asset('cyh.png') }}" alt="Logo" style="max-height: 80px; width: auto;">
        @endif
    </div>
