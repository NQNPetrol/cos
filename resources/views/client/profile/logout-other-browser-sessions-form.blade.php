<x-action-section>
    <x-slot name="title">
        <span class="text-gray-300">{{ __('Sesiones del Navegador') }}</span>
    </x-slot>

    <x-slot name="description">
        <span class="text-gray-400">{{ __('Gestiona y cierra sesión en tus sesiones activas en otros navegadores y dispositivos.') }}</span>
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div class="flex items-center mt-5">
            ...
            <x-action-message class="ms-3 text-gray-300" on="loggedOut">
                {{ __('Hecho.') }}
            </x-action-message>
        </div>
        ...
        <x-dialog-modal wire:model.live="confirmingLogout">
            <x-slot name="title">
                <span class="text-gray-800">{{ __('Cerrar Sesión en Otras Sesiones del Navegador') }}</span>
            </x-slot>

            <x-slot name="content">
                <span class="text-gray-700">{{ __('Por favor, ingresa tu contraseña para confirmar que deseas cerrar sesión en tus otras sesiones del navegador en todos tus dispositivos.') }}</span>
            ...
            <x-slot name="footer">
                ...
                <x-button class="ms-3"
                    ...
                    {{ __('Cerrar Sesión en Otras Sesiones') }}
                </x-button>
            </x-slot>
        </x-dialog-modal>
    </x-slot>
